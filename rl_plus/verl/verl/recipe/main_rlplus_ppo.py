...
from verl.recipe.reward_with_format import deepscaler_reward_fn_impl1
from verl.recipe.math_verify_reward import reward_fn_math_verify, reward_fn_math_verify_no_think
...
@hydra.main(config_path='config', config_name='rlplus_ppo_trainer', version_base=None)
def main(config):
...
def main_task(config):
    ...
    from .rlplus_fsdp_worker import RLPlusActorRolloutRefWorker
    ...
    if config.actor_rollout_ref.ref.use_ref:
        role_worker_mapping = {
            Role.ActorRollout: ray.remote(RLPlusActorRolloutRefWorker),
            Role.Critic: ray.remote(CriticWorker),
            Role.RefPolicy: ray.remote(RLPlusActorRolloutRefWorker)
        }
        mapping = {
    ...
    from .rlplus_trainer import RLPlusRayPPOTrainer
    if not config.trainer.acc_rebatch:
        trainer = RLPlusRayPPOTrainer(config=config,
                                tokenizer=tokenizer,
        ...
        from .rlplus_trainer_acc_rebatch import RLPlusRayPPOTrainerAccRebatch
        trainer = RLPlusRayPPOTrainerAccRebatch(config=config,
                                tokenizer=tokenizer,
                                role_worker_mapping=role_worker_mapping,
                                resource_pool_manager=resource_pool_manager,
                                ray_worker_group_cls=ray_worker_group_cls,
